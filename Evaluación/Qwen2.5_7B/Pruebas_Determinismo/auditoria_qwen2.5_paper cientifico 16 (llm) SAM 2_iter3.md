...
| 📅 **Fecha de Análisis** | 2026-06-28 17:26:54 |
| 🤖 **Modelo Local** | `qwen2.5` |
| ⏳ **Tiempo de Ejecución** | 388.24s |
...
|---|--------------------|-----------|---------------------------|
| 1 | Claims | 🟢 Yes | The paper's claims in the abstract and introduction are accurately reflected by the results presented. The abstract states that SAM 2 demonstrates strong performance in both static image and video domains, which is supported by experimental results showing better segmentation accuracy compared to prior work (e.g., SAM+XMem++ with an accuracy of 56.9 vs. SAM2 with 64.7). Additionally, the introduction claims that SAM 2 can produce better segmentation accuracy while using fewer interactions than prior approaches, which is validated by experiments demonstrating a reduction in required interactions from 3 to 1 for video segmentation tasks. |
| 2 | Limitations | 🟢 Yes | The paper explicitly discusses several limitations, including difficulties with segmenting objects across shot changes, losing track of or confusing objects in crowded scenes after long occlusions, and struggling with thin or fast-moving objects. The authors also mention challenges with accurately tracking objects that have similar appearances (e.g., multiple identical juggling balls). These limitations are discussed in the 'Limitations' section. |
...
| 5 | Open Access to Data and Code | 🟢 Yes | The paper provides a URL to the GitHub repository for SAM2: `https://github.com/facebookresearch/sam2` (Section F Details on zero-shot transfer experiments). This repository likely contains the code and data necessary to reproduce the main experimental results. Additionally, the SA-V dataset is available at `https://ai.meta.com/datasets/segment-anything-video/`, which is a public URL that satisfies the open access requirement. |
| 6 | Experimental Setting / Details | 🟢 Yes | The paper specifies important details about the experimental setup, such as data splits (Section F Details on zero-shot transfer experiments), hyperparameters like the optimizer and learning rate (Section D SAM2details), and training steps (Section F Details on zero-shot transfer experiments). While some minor optimizer internals are not explicitly mentioned, primary hyperparameters are present. |
...
| 8 | Experiments Compute Resource | 🟢 Yes | The paper provides sufficient details about the compute resources used for experiments. Specifically, it mentions that SAM2 runs at real-time speeds on an A100 GPU with a batch size of one, achieving 43.8 and 30.2 FPS for Hiera-B+ and Hiera-L models respectively. Additionally, the paper reports the total time required to run certain experiments, such as the SA-V val/test sets which took around 196 hours. |
| 9 | Code of Ethics | 🟢 Yes | The paper mentions 'See Ethical considerations and license for restrictions.' in the Code of Ethics section. Although there is no explicit detailed statement, this indicates that the authors are aware of ethical considerations and have provided a reference to further information. |
| 10 | Broader Impacts | 🟢 Yes | The paper discusses potential negative societal impacts in the 'Limitations' section. Specifically, it mentions that when using SAM 2 in new settings, researchers should perform their own fairness evaluation for SAM 2 specific to their use case. This indicates an awareness of broader implications and a suggestion for further ethical consideration. |
| 11 | Safeguards | 🔵 N/A | The paper does not present a high-risk artefact that could be misused for generating harmful content, enabling surveillance, synthesising dangerous information, or being weaponised. The work focuses on improving video segmentation capabilities and does not involve any direct path to misuse as defined by the safeguards criteria. Therefore, it is not applicable to require explicit access restrictions, usage guidelines, or technical guardrails. |
| 12 | Licenses | 🔴 No | The paper uses existing assets such as the SA-V dataset and mentions a license for it (Creative Commons Attribution 4.0 International Public License). However, there is no mention of an Apache 2.0 license in the provided summary or relevant sections. The absence of this specific license indicates that the authors have not respected the conditions required by Item 12 of the NeurIPS 2026 official criteria. |
| 13 | Assets | 🔵 N/A | The paper does not appear to introduce any new datasets, models, or benchmarks that it created as part of this work. The authors mention using existing datasets such as SA-V Manual, SA-V Manual+Auto, Internal-test, and EgoExo4D for their experiments. They also reference the use of a pre-trained model called Hiera-B+. Since no new assets are being released or documented by the authors, Item 13 does not apply. |
