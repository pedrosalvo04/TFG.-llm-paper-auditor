...
| 📅 **Fecha de Análisis** | 2026-06-24 20:02:57 |
| 🤖 **Modelo Local** | `qwen2.5` |
| ⏳ **Tiempo de Ejecución** | 540.07s |
...
| 3 | Theory, Assumptions & Proofs | 🔵 N/A | The provided information does not contain any theoretical results, assumptions, or proofs. The paper focuses on the practical aspects of the SAM2 model and its application in segmenting images and videos. Given that there are no theoretical contributions mentioned, it is not applicable to evaluate whether the theory, assumptions, and proofs are transparently stated as per NeurIPS 2026 criteria. |
| 4 | Experimental Result Reproducibility | 🟢 Yes | The paper provides a code URL (https://github.com/facebookresearch/sam2) and a data URL (https://ai.meta.com/datasets/segment-anything-video/) that allow others to access the authors' own implementation or data used for the main experiments. This satisfies the requirement of making results reproducible, as per NeurIPS 2026 criteria. |
...
| 6 | Experimental Setting / Details | 🟢 Yes | The paper provides detailed information about the training details in the 'hyperparameters' section. For example, it mentions the optimizer used (AdamW) and batch sizes for different stages of training: pre-training with a batch size of 128 and video tasks with a batch size of 1. The full details can be found at https://github.com/facebookresearch/sam2. |
| 7 | Experiment Statistical Significance | 🔴 No | The paper does not provide any information about error bars, confidence intervals, or statistical significance tests. The relevant sections of the paper and the provided JSON summary do not mention any statistical measures that would indicate the reliability or reproducibility of the experiments. According to the NeurIPS 2026 criteria, this is a requirement for reporting the results of experiments supporting the main claims of the paper. Without these measures, it is impossible to assess the statistical significance of the reported findings. <br><br> ⚠️ *Falta justificación explícita del autor para la respuesta 'No'* |
| 8 | Experiments Compute Resource | 🟢 Yes | The paper provides detailed information about the hardware used for training and inference, including the number of A100 GPUs, their configuration, and the total energy consumption. Specifically, it mentions '256 A100 GPUs' for training infrastructure, 'Single A100 GPU (80GB)' for inference, along with details on energy consumption ('12165.12 kWH') and carbon emissions ('3.89 metric tons of CO2e'). This information is sufficient to reproduce the experiments in terms of hardware requirements. The criteria state that mentioning any hardware or cluster, including environmental impact metrics like CO2 emissions, is compliant. Therefore, based on the provided details, the paper meets the transparency requirement for compute resources. |
...
| 12 | Licenses | 🟢 Yes | The paper explicitly states the licensing terms for the released assets in the 'H.3 Data annotation card' and the associated repository documentation: 'SA-V dataset: CC by 4.0' and 'SAM 2 code: Apache 2.0'. This provides clear information on how users can access and use the model and data. |
